...
for frame in range(init1,len(traj),step):
    ...
    min_val=numpy.min(diff) # Minimum of differences means, the values of densities were close enough
    min_val1=numpy.min(diff1) # Minimum of differences means, the values of densities were close enough
    
    max_val=numpy.max(diff) # Minimum of differences means, the values of densities were close enough
    max_val1=numpy.max(diff1) # Minimum of differences means, the values of densities were close enough
    ...
    derivative0  = sg.d(diff,box_x[0:Len])
    derivative1  = sg.d(diff1,box_x[Len:])
...
filename3 = "width_vs_time1"+"_Isodiamond_network_diffusive"+"_Ntot_"+str(Ntot)+"_vol_frac"+str(fraction)+"_densratio_"+str(densratio)+"_molfrac_"+str(molefrac)+"_kT_"+str(kbT)+".txt"
