...
for sample in range(1,samples):
    ...
    for frame in range(init1,len(traj)-1):
        ...
        for x in range(totalcell):
            ...
            msdx=numpy.nanmean(numpy.power((real_pos[1,:]-real_pos[0,:]),2),axis=0)
            msdy=numpy.nanmean(numpy.power((real_posy[1,:]-real_posy[0,:]),2),axis=0)
            msdz=numpy.nanmean(numpy.power((real_posz[1,:]-real_posz[0,:]),2),axis=0)
...
data9 = numpy.asarray((msd_samplebin[:,:,0].T,msd_samplebin[:,:,1].T,msd_samplebin[:,:,2].T,msd_samplebin[:,:,0].T/(2*period*dt),msd_samplebin[:,:,1].T/(2*period*dt),msd_samplebin[:,:,2].T/(2*period*dt))).T
data10= numpy.asarray((msd_samplevar[:,:,0].T,msd_samplevar[:,:,1].T,msd_samplevar[:,:,2].T,msd_samplevar[:,:,0].T/(2*period*dt),msd_samplevar[:,:,1].T/(2*period*dt),msd_samplevar[:,:,2].T/(2*period*dt))).T
...
#print(data1)
filename3 = "Monomer_Positiondep_diffusionconst_vs_xbox"+"_Isodiamond_network_diffusive"+"_Ntot_"+str(Ntot)+"_vol_frac"+str(fraction)+"_densratio_"+str(densratio)+"_molfrac_"+str(molefrac)+"_kT_"+str(kbT)+".txt"
