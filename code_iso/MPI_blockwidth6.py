...
filename3 = "Subbox_density_vs_xboxtrial"+str(sample)+"_diamond_network_diffusive"+"_Ntot_"+str(Ntot)+"_vol_frac"+str(fraction)+"_densratio_"+str(densratio)+"_molfrac_"+str(molefrac)+"_kT_"+str(kbT)+".txt"
with open(filename3, 'w+') as f3:                                                                                                                              
    ...
    for k in range(0,len(nbin_array)):
        ...
        data2 = numpy.zeros((int((end_index-start_index)/step),int(cellnoz*cellnoy),int(cellnox),3),dtype=float)
        for frame in range(start+start_index,start+end_index,step):
            ...
            data1 = numpy.empty((0,int(cellnox),3),dtype=float)
            for subbox in range(cellnoy*cellnoz):
                ...
                data = numpy.asarray((bin_val.T,densityA.T,densityB.T)).T
                data1 = numpy.append(data1,[data],axis=0)
                            
            data2[int((frame -(start+start_index))/step)] = data1
        ...
        if rank==0:
            ...
            print("datatime_concatmean.shape",datatime_concatmean.shape)
                #print(datatime_concatmean)
            tmp = numpy.nanmean(datatime_concatmean,axis=0)
            print("tmp.shape",tmp.shape)
            width = numpy.empty(0,dtype=float)
            xrange1 =  numpy.arange(tmp[0,0,0],tmp[0,cellnox//2,0],1)

            xrange2 = numpy.arange(tmp[0,cellnox//2,0],tmp[0,cellnox-1,0],1)
            for subbox  in range(int(cellnoy*cellnoz)):
                poly1st  = sm.nonparametric.lowess(tmp[subbox,:cellnox//2,2],tmp[subbox,:cellnox//2,0], frac=0.08,xvals=xrange1).T
                gel1st = sm.nonparametric.lowess(tmp[subbox,:cellnox//2,1],tmp[subbox,:cellnox//2,0],frac=0.08,xvals =xrange1).T
                poly2nd = sm.nonparametric.lowess(tmp[subbox,cellnox//2:,2],tmp[subbox,cellnox//2:,0],frac=0.08,xvals =xrange2).T
                gel2nd = sm.nonparametric.lowess(tmp[subbox,cellnox//2:,1],tmp[subbox,cellnox//2:,0],frac=0.08,xvals =xrange2).T
                print("gel2nd.shape:",gel2nd.shape)
                
                maxpoly2 = numpy.max(poly2nd)
                minpoly2 = numpy.min(poly2nd)
                maxgel2 = numpy.max(gel2nd)
                mingel2 = numpy.min(gel2nd)

                maxpoly1 = numpy.max(poly1st)
                minpoly1 = numpy.min(poly1st)
                maxgel1 = numpy.max(gel1st)
                mingel1 = numpy.min(gel1st)
               
                derivativepoly1 = numpy.gradient(poly1st,xrange1)
                derivativegel1 =  numpy.gradient(gel1st,xrange1)
                derivativepoly2 = numpy.gradient(poly2nd,xrange2)
                derivativegel2 =  numpy.gradient(gel2nd,xrange2)
                ...
                width_ = numpy.nanmean(numpy.asarray([width1,width2]))
                width = numpy.append(width,width_)
                print("width.shape",width.shape)
            width2 = width*width
            width2mean = numpy.nanmean(width2)


            data_nbin[k,0] = nbin
            data_nbin[k,1] = cutoffz
            data_nbin[k,2] = width2mean
