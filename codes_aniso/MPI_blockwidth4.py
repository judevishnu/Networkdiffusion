...
with open(filename3, 'w+') as f3:                                                                                                                              
    ...
    for k in range(0,len(nbin_array)):
        ...
        #print("rank : cellno_z,cellno_y,cellno_x",rank,cellnoz,cellnoy,cellnox)
        ...
        for frame in range(start+start_index,start+end_index,step):
            ...
            for subbox in range(cellnoy*cellnoz):
                ...
                poly1st  = sm.nonparametric.lowess(data[:len(data)//2,2],data[:len(data)//2,0], frac=0.2).T
                gel1st = sm.nonparametric.lowess(data[:len(data)//2,1],data[:len(data)//2,0],frac=0.2).T
                poly2nd = sm.nonparametric.lowess(data[len(data)//2:,2],data[len(data)//2:,0],frac=0.2).T
                gel2nd = sm.nonparametric.lowess(data[len(data)//2:,1],data[len(data)//2:,0],frac=0.2).T
