...
for frame in range(init+start_index,init+end_index,step):
        ...
        clids = cl.cluster_idx[N_constraint:]
        #print("clids",clids.shape)
        selclusterids =clids.reshape(-1)
        clusterkeys = numpy.asarray(cl.cluster_keys[1:])

        selpolyid = numpy.where(((compoly[:,0]>leftregion) & (compoly[:,0]<rightregion)))[0]
        selclusterkeys = clusterkeys[selpolyid]
        newclid = clids.reshape(-1,poly_len)[selpolyid].reshape(-1)
        selpoints = points[selclusterkeys]
        selpoints = selpoints.reshape(-1,3)
        particletag = numpy.arange(0,len(selpoints))
        particletagpolyid = particletag.reshape(-1,poly_len)
        #n_bonds = numpy.lib.stride_tricks.sliding_window_view(particletagpolyid,2,axis=1).reshape(-1,2)
        #print(n_bonds.shape,particletagpolyid.shape)
        aq = freud.locality.AABBQuery(box, selpoints)
        query_result = aq.query(selpoints, dict(r_max=rmax, r_min=0.1))
        nlist = query_result.toNeighborList()
        #print("prefilter",len(nlist))
        #nlist.filter(newclid[nlist.point_indices]!=newclid[nlist.query_point_indices])
        #print("postfilter",len(nlist))
        nlist = numpy.asarray(nlist)
        
        #print(nlist.shape)
        #n_bonds = numpy.append(n_bonds,nlist,axis=0)
        #n_bonds=numpy.sort(n_bonds,axis=1)
        #n_bonds=numpy.unique(n_bonds,axis=0)
        n_bonds = nlist
        #print(n_bonds.shape)

        if selpoints.size!=0:
            system = freud.AABBQuery.from_system((box,selpoints))
            distances = numpy.linalg.norm(box.wrap(selpoints[n_bonds[:, 1]] - selpoints[n_bonds[:, 0]]),axis=1)
            neighbors = freud.locality.NeighborList.from_arrays(len(selpoints),len(selpoints),n_bonds[:, 0],n_bonds[:, 1],distances,)

            cl2 = freud.cluster.Cluster()
            cl2.compute(system=system, neighbors=neighbors)
        ...
        for u in range(num_cluster):
            ...
            if (len(unique_id)>=int(poly_len)):
                numclust+=1
                #print("len(uniqueid)",len(unique_id))
                trutharray1[u]=True
            ...
            if comptruth:
                print("nettruth1 :",nettruth1,"nettruth2 :",nettruth2,"nettruth3 :",comptruth)
            #exit()
        ...
        #print(trutharray1,numpy.logical_not(trutharray))
        ...
        data2[int((frame-(init+start_index))/step),3]=cl2.num_clusters
        ...
        #print("Sizes:",cl_props2.sizes)
        #print("rgofcl:",cl_props2.gyrations[:,0,0],cl_props2.gyrations[:,1,1],cl_props2.gyrations[:,2,2])
        #print("rgofcl:",cl_props2.gyrations[:,0,0])
