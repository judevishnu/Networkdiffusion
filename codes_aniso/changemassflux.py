...
def flux(x,mA,mB,velA,velB):
     ...
     avvelABxyz =  numpy.nansum(numpy.asarray([rhoA*numpy.nanmean(velxyzA,axis=0),rhoB*numpy.nanmean(velxyzB,axis=0)]))/rho
     jAxyz = rho*(numpy.nanmean(velxyzA - avvelABxyz,axis=0))
     jBxyz = rho*(numpy.nanmean(velxyzB - avvelABxyz,axis=0))
     return jAxyz,jBxyz
...
for sample in range(1,samples):
    ...
    for frame in range(init1,len(traj)-1):
        ...
        for x in range(totalcell):
            jAxyz,jBxyz=flux(x,mA,mB,velA,velB)
            jAplusxyz,jBplusxyz=flux(x,mAplus,mBplus,velAplus,velBplus)
            djAxyz = jAplusxyz - jAxyz 
            djBxyz = jBplusxyz - jBxyz 

            d_bin=numpy.append(d_bin,numpy.asarray((djAxyz[0],djAxyz[1],djAxyz[2],djBxyz[0],djBxyz[1],djBxyz[2])).reshape(-1,6),axis=0)  
...
#print(data1)
filename3 = "ChangeMassflux_Positiondep_diffusionconst_vs_xbox"+"_diamond_network_diffusive"+"_Ntot_"+str(Ntot)+"_vol_frac"+str(fraction)+"_densratio_"+str(densratio)+"_molfrac_"+str(molefrac)+"_kT_"+str(kbT)+".txt"
