    ...
    for sample in range(1,samples):
        ...
        for frame in range(start,len(traj),step):
            ...
            for subbox in range(cellnoy*cellnoz):
                ...
                poly1st = CubicSpline(data[:len(data)//2,0],data[:len(data)//2,2])
                gel1st = CubicSpline(data[:len(data)//2,0],data[:len(data)//2,1])
                poly2nd = CubicSpline(data[len(data)//2:,0],data[len(data)//2:,2])
                gel2nd = CubicSpline(data[len(data)//2:,0],data[len(data)//2:,1])
                x1st = numpy.arange(bin_val[0],bin_val[len(bin_val)//2],0.05)
                x2nd = numpy.arange(bin_val[len(bin_val)//2],bin_val[len(bin_val)-1],0.05)
                #print(x1st)
                #exit()
                #derivativepoly1 = poly1st(x1st)
                #derivativegel1 = gel1st(x1st)
                #derivativepoly2 = poly2nd(x2nd)
                #derivativegel2 = gel2nd(x2nd)

                derivativepoly1 = savgol_filter(data[:len(data)//2,2], window_length=5, polyorder=4, deriv=1)
                derivativegel1 = savgol_filter(data[:len(data)//2,1], window_length=5, polyorder=4, deriv=1)
                derivativepoly2 = savgol_filter(data[len(data)//2:,2], window_length=5, polyorder=4, deriv=1)
                derivativegel2 = savgol_filter(data[len(data)//2:,1], window_length=5, polyorder=4, deriv=1)
                
                
                
                maxpoly1 = numpy.max(data[:len(data)//2,2])
                minpoly1 = numpy.min(data[:len(data)//2,2])
                maxgel1 = numpy.max(data[:len(data)//2,1])
                mingel1 = numpy.min(data[:len(data)//2,1])
            
                maxpoly2 = numpy.max(data[len(data)//2:,2])
                minpoly2 = numpy.min(data[len(data)//2:,2])
                maxgel2 = numpy.max(data[len(data)//2:,1])
                mingel2 = numpy.min(data[len(data)//2:,1])
