...
d_samplebin = numpy.empty((0,totalcell,3),dtype=float)
d_samplevar = numpy.empty((0,totalcell,3),dtype=float)
for sample in range(1,samples):
    ...
    data1 = numpy.empty((0,len(bin_val),5),dtype=float)
    d_bintime=numpy.empty((0,totalcell,3),dtype=float)
    for frame in range(init1,len(traj)-1):
        d_bin=numpy.empty((0,3),dtype=float)
        container=[]
        ...
        for x in range(totalcell):
            ...
            velxyz = numpy.take(velB,idsBplus,axis=0)
            ...
            #j1=rhoBplus*(numpy.square(box.wrap(posxyzBplus - posxyzB)))/(dt*period)
            #avgJ1 = numpy.nanmean(j,axis=0)
            avvxyz = numpy.nanmean(velxyz-vcom,axis=0)

            d_bin=numpy.append(d_bin,numpy.asarray((avvxyz[0],avvxyz[1],avvxyz[2])).reshape(-1,3),axis=0)  
        print(d_bin.shape) 
...
data9 = numpy.asarray((d_samplebin[:,:,0].T,d_samplebin[:,:,1].T,d_samplebin[:,:,2].T)).T
data10= numpy.asarray((d_samplevar[:,:,0].T,d_samplevar[:,:,1].T,d_samplevar[:,:,2].T)).T
...
#print(data1)
filename3 = "Velprofile_vs_xbox"+"_diamond_network_diffusive"+"_Ntot_"+str(Ntot)+"_vol_frac"+str(fraction)+"_densratio_"+str(densratio)+"_molfrac_"+str(molefrac)+"_kT_"+str(kbT)+".txt"
