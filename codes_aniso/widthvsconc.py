...
widthsample = numpy.zeros((samples-1,2),dtype=float)
step=1
start =1550
for sample in range(1,samples):
    ...
    for frame in range(start+start_index,end_index+start,step):
        ...
        poly1st  = sm.nonparametric.lowess(data[:len(data)//2,2],data[:len(data)//2,0], frac=0.11).T
        gel1st = sm.nonparametric.lowess(data[:len(data)//2,1],data[:len(data)//2,0],frac=0.11).T
        poly2nd = sm.nonparametric.lowess(data[len(data)//2:,2],data[len(data)//2:,0],frac=0.11).T
        gel2nd = sm.nonparametric.lowess(data[len(data)//2:,1],data[len(data)//2:,0],frac=0.11).T
    ...
    if rank==0:
        ...
        widthtimestd=numpy.nanstd(data_concatmean,axis=0)
        
        print("sample:",sample,"widthtimemean:",widthtimemean)
        widthsample[sample-1,0]=widthtimemean[0]
        widthsample[sample-1,1]=widthtimestd[0]
    comm.Barrier()
...
if rank==0:
    filename2 = "width2_vs_conc3"+"_diamond_network_diffusive"+"_Ntot_"+str(Ntot)+"_vol_frac"+str(fraction)+"_densratio_"+str(densratio)+"_molfrac_"+str(molefrac)+"_kT_"+str(kbT)+".txt"
    print(widthsample)
