...
filename3 = "Subbox_density_vs_xboxtryquant"+str(sample)+"_diamond_network_diffusive"+"_Ntot_"+str(Ntot)+"_vol_frac"+str(fraction)+"_densratio_"+str(densratio)+"_molfrac_"+str(molefrac)+"_kT_"+str(kbT)+".txt"
with open(filename3, 'w+') as f3:                                                                                                                              
    ...
    for k in range(0,len(nbin_array)):
        ...
        for frame in range(start+start_index,start+end_index,step):
            ...
            xrang1 = numpy.linspace(bin_val[0],bin_val[cellnox//2],10)
            xrang2 = numpy.linspace(bin_val[cellnox//2],bin_val[cellnox-1],10)
            for subbox in range(cellnoy*cellnoz):
                ...
                #poly1stf  = sm.QuantReg(data[:len(data)//2,2],sm.add_constant(data[:len(data)//2,0])).fit(q=0.15,max_iter=1000)
                #gel1stf = sm.QuantReg(data[:len(data)//2,1],sm.add_constant(data[:len(data)//2,0])).fit(q=0.15,max_iter=1000)
                #poly2ndf = sm.QuantReg(data[len(data)//2:,2],sm.add_constant(data[len(data)//2:,0])).fit(q=0.15,max_iter=1000)
                #gel2ndf = sm.QuantReg(data[len(data)//2:,1],sm.add_constant(data[len(data)//2:,0])).fit(q=0.15,max_iter=1000)
